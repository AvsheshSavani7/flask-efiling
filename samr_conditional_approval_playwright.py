...
def get_deals_from_mongodb(include_conditional=False):
    ...
    Args:
        include_conditional: If False, only return deals that don't have a 'samr_approval' node
    ...
    try:
        ...
        # Build query - exclude deals with 'samr_approval' node if include_conditional is False
        query = {}
        if not include_conditional:
            query = {"samr_approval": {"$exists": False}}
        ...
        filter_msg = "without 'samr_approval' node" if not include_conditional else "all"
...
def load_deals(include_conditional=False):
    ...
    Args:
        include_conditional: If False, only load deals that don't have a 'samr_approval' node
    """
    global deals
...
def detail_url_exists_in_conditional_data(detail_url):
    """
    Check if detail_url already exists in any deal's samr_approval data.
    ...
    try:
        ...
        # Search for deals where samr_approval.url matches
        query = {"samr_approval.url": detail_url}
...
def match_deal_with_llm(title_en, title_cn):
    ...
    # Only load deals without 'samr_approval' node to avoid re-processing
    if not deals:
        print("⚠️ Deals list is empty, reloading from MongoDB (excluding deals with 'samr_approval' node)...")
...
def save_conditional_approval_data_to_deal(deal_match, matched_result):
    """
    Save matched result to MongoDB deal record under 'samr_approval' node.
    ...
    try:
        ...
        # Update the deal document with samr_approval data
        # Also mark as conditionally_approved
        update_result = collection.update_one(
            query,
            {
                "$set": {
                    "samr_approval": conditional_data_serializable,
...
def main(use_existing_html=False, headless=True):
    ...
    # Only load deals without 'samr_approval' node to avoid re-processing
    print("📊 Loading deals from MongoDB (excluding deals with 'samr_approval' node)...")
    load_deals(include_conditional=False)
    ...
    # Filter out records that are already processed (url exists in samr_approval data)
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    if deal_match:
                        ...
                        # Save to MongoDB under 'samr_approval' node in the deal record
