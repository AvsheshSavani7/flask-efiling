...
from new_fs_cases_html import match_case_to_deal, match_case_to_deal_partial
...
_SCRAPE_FIELDS = frozenset({
    ...
    "initiation_date", "provisional_deadline", "economic_activities",
    "simplified_procedure", "case_notified_under", "decisions",
...
def update_case_document(
    ...
    try:
        ...
        if missing:
            ...
            logger.info(
                f"    [{case_number}] Unsetting vanished scrape fields: {missing}")
...
def run(headed: bool = False, max_cases: Optional[int] = None):
                    ...
                    if deal:
                        ...
                        if deals and companies:
                            try:
                                ...
                                logger.exception(
                                    f"Step 8.13c: [{case_number}] Partial deal match error: {e}")
                                collect_error(
                                    error_items,
                                    str(e),
                                    case_number=case_number,
                        ...
                        if partial_match:
                            ...
                            logger.info(
                                "Step 8.13c: [%s] Partial match (deal_id=%s side=%s) "
    ...
    logger.info(f"Step 8.23b: Partial one-side matches    : {partial_match_count}")
    logger.info(
