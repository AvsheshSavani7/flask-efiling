"""
EC Foreign Subsidies (FS) Cases Re-Analysis Script
====================================================
Re-runs LLM deal matching and USA-relation checks for fs_cases records
that were inserted on/after 2026-05-22 without analysis (due to a bad API key).

What it does:
1. Queries fs_cases for records where:
   - created_at >= CUTOFF_DATE
   - deal_id is null/missing/empty
   - reanalyzed_at is absent (not yet re-processed)
2. For each record:
   - Resolves companies list (from 'companies' field; falls back to case_title split)
   - Runs LLM deal matching using the companies list
...
Usage:
    python fs_reanalyze.py
    python fs_reanalyze.py --cutoff 2026-05-22
    python fs_reanalyze.py --dry-run        # logs only, no DB writes or emails
"""
...
from new_fs_cases_html import (
    fetch_deals,
    generate_matched_email,
    generate_usa_email,
    get_companies_from_title,
    get_fs_cases_collection,
    match_case_to_deal,
...
SCRIPT_NAME = "fs_cases_reanalyze"
...
def update_record_after_analysis(
    ...
    """
    Update an fs_cases record after re-analysis:
...
def reanalyze(cutoff_str: str, dry_run: bool) -> Dict[str, Any]:
    ...
    logger.info("=" * 60)
    logger.info(f"[STEP 1] EC Foreign Subsidies (FS) Cases Re-Analysis")
    ...
    if collection is None:
        _log_critical_error_and_email(
            "fs_cases collection not available", {"step": "get_fs_cases_collection"}
        )
        return {"success": False, "error": "fs_cases collection unavailable"}
    ...
    for idx, record in enumerate(records, 1):
        ...
        # FS pages often store companies=null; fall back to deriving from case_title
        companies: List[str] = get_companies_from_title(record)
        ...
        if not companies:
            logger.warning(f"  {case_number}: No companies resolvable (title={case_title[:60]}), skipping LLM match")
            update_record_after_analysis(collection, case_number, None, dry_run)
            continue

        logger.info(f"  {case_number}: companies={companies}")
        ...
        if match_result:
            ...
            if deal:
                ...
                # Send [FRMD] email — FS version passes companies as 3rd arg
                subject, html_email = generate_matched_email(record, deal, companies)
                if dry_run:
                ...
                logger.warning(
                    f"  {case_number}: LLM returned deal_id={matched_deal_id} but deal not found anywhere; "
        ...
        if is_usa:
            ...
            # FS version passes companies as 2nd arg
            subject, html_email = generate_usa_email(record, companies)
            if dry_run:
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-run LLM analysis on fs_cases records missed due to bad API key."
    )
    parser.add_argument(
        "--cutoff",
