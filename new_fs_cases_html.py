...
def scrape_case_detail(context, url: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if parsed and not parsed.get("error"):
            ...
            logger.warning(f"  [{case_num}] Parse returned error: {parsed.get('error') if parsed else 'None'}")
        return parsed
    except Exception as exc:
        _log_error_and_email(
...
def case_exists(collection, case_number: str) -> bool:
    try:
        exists = collection.count_documents({"case_number": case_number}, limit=1) > 0
        return exists
