...
from n8n_email_service import get_docket_recipients
...
def home():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "/ukraine-amcu-scraper": "GET - Ukraine AMCU merger tracker (live last 2 days). Deal match then USA-related email to avshesh only. Query: backfill, no_deal_match. Logs: /logs?script=ukraine_amcu_cases",
...
def nj_bpu_scraper_endpoint():
    ...
    GET params or POST JSON body:
        case_id:        NJ BPU case_id (optional). Omit to run all active dockets from nj_bpu_dockets.json.
        docket_number:  Docket number (required when case_id is provided, e.g. TM26030047)
    ...
    Modes:
        - Omit case_id → run all active dockets from docket_engine/nj_bpu_dockets.json
        - Provide case_id + docket_number → run a single specific docket
    """
    try:
        if request.method == 'POST':
            data = request.get_json(silent=True) or {}
        ...
        if not case_id:
            ...
            logger.info(
                f"Starting NJ BPU scraper (all dockets from config), test_mode={test_mode}"
            )
        ...
        elif case_id and docket_number:
            # Run a single specific docket
            logger.info(
            ...
            return jsonify({
                "success": False,
                "error": "Provide both case_id and docket_number for a single docket, "
                         "or omit case_id to run all dockets from nj_bpu_dockets.json."
            }), 400
...
@app.route('/chile-fne-scraper', methods=['GET'])
def chile_fne_scraper_endpoint():
    """
    Scrape Chile FNE merger filings, match cases with deals,
    and store results in the 'chili_cases' collection.
    Process runs in background - returns immediately.
    ...
    try:
        ...
        def run_scraper():
            try:
                logger.info(
                    f"Starting Chile FNE scraper in background (backfill={backfill})")
                run_chile_fne_cases_register(backfill=backfill)
                logger.info("Chile FNE scraper completed successfully.")
            except Exception as e:
                logger.exception("Error in background Chile FNE scraper")

        task_name = "chile-fne-scraper-backfill" if backfill else "chile-fne-scraper"
        submitted, msg = submit_unique_task(task_name, run_scraper)
        if not submitted:
            return jsonify({"success": False, "error": msg, "status": "already_running"}), 409
...
def ukraine_amcu_scraper_endpoint():
    """
    Live Ukraine AMCU merger tracker. Polls the news timeline (last 2 days),
    splits concentration items, updates ukraine_cases, and emails new/update
    alerts to avshesh.savani@teqnodux.com (deal match → FRMD, else USA → FRUD).
    Runs in the background.

    Query parameters:
        backfill: if true, scrape from 2026-01-01 (no email).
        no_deal_match: if true, skip deal_id LLM/regex (USA/FRUD still runs on live).
    """
    try:
        backfill = request.args.get(
            "backfill", "false").lower() in ("true", "1", "yes")
        ...
        def run_scraper():
            try:
                logger.info(
                    ...
                    "(backfill=%s no_deal_match=%s)",
                    backfill, no_deal_match,
                )
        ...
        return jsonify({
            ...
            "backfill": backfill,
            "no_deal_match": no_deal_match,
            "email_recipient": "avshesh.savani@teqnodux.com" if not backfill else None,
        }), 200
