...
def generate_update_email_html(
    ...
    if deal_match:
        ...
        subject = build_subject("uk_cma", "update", deal_match)
        title_text = (
        ...
        if not deal_id:
            ...
            if deal_match:
                ...
                if partial_match:
                    ...
                    print(f"STEP 1.5.20b: FRPMD email sent: {subj}")
                else:
                    try:
...
def main():
    ...
    updated_count = 0
    unchanged_count = 0
    match_stats = {"llm": 0, "regex": 0, "partial": 0}
    ...
    try:
        ...
        logger.info(
            f"STEP 1.6.5c: Partial one-side matches    : {match_stats['partial']}")
        logger.info(
